...
parser.add_argument(
    "--prompt", type=str, nargs="?", default="DEFAULT PLACEHOLDER PROMPT PLEASE IGNORE", help="the prompt to render"
)
...
parser.add_argument("--img-latent", type=str, nargs="?", help="path to the latent you want to make")
parser.add_argument("--name", type=str, nargs="?", default="latent2img_outputImg")
    ...
    for n in trange(opt.n_iter, desc="Sampling"):
        for prompts in tqdm(data, desc="data"):
            ...
            with precision_scope("cuda"):
                ...
                if (Path(opt.img_latent).exists()):
                    with open(opt.img_latent, "r") as json_file:
                        latent_list = json.load(json_file)
                    latent_ndarray = np.asarray(latent_list)
                    init_latent = torch.from_numpy(latent_ndarray).to(torch.float32).to("cuda")
                ...
                for i in range(batch_size):
                    ...
                    Image.fromarray(x_sample.astype(np.uint8)).save(
                        os.path.join(sample_path, f"{opt.name}.{opt.format}")
                    )
                    seeds += str(opt.seed) + ","
                    opt.seed += 1
                    base_count += 1
